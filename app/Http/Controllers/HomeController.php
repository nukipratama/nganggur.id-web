...
class HomeController extends Controller
{
    public function index()
    {
        if (auth()->user()->role_id === 1) {
            ...
            $badge['total'] =  Project::where('partner_id', auth()->id())->count();
            $badge['ongoing'] =  Project::where([['partner_id', '=', auth()->id()], ['status_id', '<', '4']])->count();
            $badge['success'] =  Project::where([['partner_id', '=', auth()->id()], ['status_id', '>', '3'], ['status_id', '<', '100']])->count();
            $badge['failed'] =  Project::where([['partner_id', '=', auth()->id()], ['status_id', '>=', '100']])->count();
            $user = User::where('id', auth()->id())->with(['details', 'role', 'type'])->first();
            $user->badge = $badge;
